...
func _physics_process( delta : float ) -> void:
	if state == State.THROW:
		...
		direction = global_position.direction_to( player.global_position + Vector2(0, 16) ) #added vector2 to change from head to torso (my issue)
		speed += acceleration * delta
		position += direction * speed * delta
		if global_position.distance_to( player.global_position + Vector2(0, 16) ) <= 10:
