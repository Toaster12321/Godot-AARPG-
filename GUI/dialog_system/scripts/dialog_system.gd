...
func _unhandled_input(event: InputEvent) -> void:
	...
	if(
		...
		if text_in_progress == true: #prevents player from skipping text until end
			content.visible_characters = text_length 
			timer.stop()
			text_in_progress = false
			show_dialog_button_indicator( true )
			return
		elif waiting_for_choice == true: #ignores key presses if waiting for choice
			return
		dialog_item_index += 1
...
#set dialog UI choice based on parameters
...
#shows NEXT button and END when text finishes
