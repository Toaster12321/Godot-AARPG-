...
@export var throw_height_strength : float = 80.0
...
func _physics_process( delta : float) -> void:
	...
	#check to see if sprite hits "ground" then destroy 
...
func disable_collisions( _node : Node ) -> void:
	for c in _node.get_children():
		if c == self: #if c is of type throwable skip, to not disable hitbox
			continue
